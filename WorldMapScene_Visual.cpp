...
void WorldMapScene::update()
{
	if (KeyEnter.down())
	{
		if (m_gameManager && m_allCities)
		{
			...
			{
				m_cutInTimer = 3.5;
				m_cutInText = U"{}年 {}月"_fmt(m_gameManager->getYear(), m_gameManager->getMonth());
				if (m == 3) {
					m_seasonText = U"春";
...
void WorldMapScene::draw() const
{
	...
	// 🗺️ 地図の本体（内側）
	// =================================================================
	RectF mapArea(40, 40, Scene::Width() - 80, Scene::Height() - 80);
	...
	// 紙のテクスチャ（細かい）
	{
		const uint32 tw = static_cast<uint32>(Max(1.0, mapArea.w));
		const uint32 th = static_cast<uint32>(Max(1.0, mapArea.h));

		// ★ サイズ変更時のみ再生成
		if (!m_paperTexture || m_lastMapWidth != tw || m_lastMapHeight != th)
		{
			m_paperTexture.emplace(tw, th, ColorF(0, 0, 0, 0));
			m_lastMapWidth = tw;
			m_lastMapHeight = th;

			m_paperTexture->clear(ColorF(0, 0, 0, 0));
			{
				const ScopedRenderTarget2D target(*m_paperTexture);

				for (int i = 0; i < 100; ++i)
				{
					double x = RandomClosed(0.0, static_cast<double>(tw));
					double y = RandomClosed(0.0, static_cast<double>(th));
		...
		if (m_paperTexture)
		{
			m_paperTexture->draw(Arg::topLeft = mapArea.pos, ColorF(1.0));
		}
	}

	const Transformer2D mapTransform{ Mat3x2::Translate(mapArea.pos) };
	...
	if (m_allCities)
	{
		...
		for (int y = 0; y < static_cast<int>(mapArea.h); y += 6)
		{
			for (int x = 0; x < static_cast<int>(mapArea.w); x += 6)
			{
				...
				for (int i = 0; i < m_allCities->size(); ++i)
				{
					Point cityPosInMap = (*m_allCities)[i].pos - mapArea.pos.asPoint();
				...
				if (closestIdx >= 0 && closestDist < 350)
				{
					...
					// 地形のグラデーション風（距離で濃淡）
					double gradientFactor = 1.0 - (closestDist / 350.0);
		...
		// =================================================================
		{
			...
			for (int i = 0; i < m_allCities->size(); ++i)
			{
				Point cityA = (*m_allCities)[i].pos - mapArea.pos.asPoint();
				...
				{
					if (i == j) continue;
					Point cityB = (*m_allCities)[j].pos - mapArea.pos.asPoint();
		...
		for (int i = 0; i < m_allCities->size(); ++i)
		{
			...
			Point cityPosInMap = c.pos - mapArea.pos.asPoint();
			Vec2 pos(cityPosInMap.x, cityPosInMap.y);
			...
			// 都市の影
			Circle(pos.movedBy(2, 2), 16).draw(ColorF(0, 0, 0, 0.4));

			// 都市の外枠（黒）
			Circle(pos, 16).draw(ColorF(0.2, 0.2, 0.2));

			// 都市の本体（勢力色）
			Circle(pos, 14).draw(factionColor);

			// 都市の内側（白い円）
			Circle(pos, 11).draw(ColorF(0.95, 0.95, 0.95));

			// 中心の点
			Circle(pos, 4).draw(factionColor);

			// 城壁の表現（4本の線）
			for (int angle = 0; angle < 360; angle += 90)
			{
				double rad = Math::ToRadians(static_cast<double>(angle));
				Vec2 start = pos + Vec2(Cos(rad), Sin(rad)) * 9;
				Vec2 end = pos + Vec2(Cos(rad), Sin(rad)) * 13;
			...
			if (isHovered)
			{
				...
				Circle(pos, 18 + sin(time * 4) * 2).drawFrame(2, ColorF(factionColor, 0.4));
			}

			// 都市名（影付き）
			FontAsset(U"menu")(c.name).drawAt(pos.x + 1, pos.y - 26, ColorF(0, 0, 0, 0.7));
			FontAsset(U"menu")(c.name).drawAt(pos.x, pos.y - 25, ColorF(0.2, 0.2, 0.2));
		}
